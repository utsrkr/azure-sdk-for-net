...
namespace Azure.Communication.Chat
{
    ...
    public class ChatThreadClient
    {
        ...
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <exception cref="RequestFailedException">The server returned an error. See <see cref="Exception.Message"/> for details returned from the server.</exception>
        public virtual async Task<Response<string>> SendMessageAsync(string content, ChatMessageType? type = null, ChatMessagePriority? priority = null, string? senderDisplayName = null, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(ChatThreadClient)}.{nameof(SendMessage)}");
            scope.Start();
            try
            {
                Response<SendChatMessageResult> sendChatMessageResult = await _chatThreadRestClient.SendChatMessageAsync(Id, content, priority, senderDisplayName, type: null, cancellationToken).ConfigureAwait(false);
        ...
        /// <param name="cancellationToken"> The cancellation token to use. </param>
        /// <exception cref="RequestFailedException">The server returned an error. See <see cref="Exception.Message"/> for details returned from the server.</exception>
        public virtual Response<string> SendMessage(string content, ChatMessageType? type = null, ChatMessagePriority? priority = null, string? senderDisplayName = null, CancellationToken cancellationToken = default)
        {
            using DiagnosticScope scope = _clientDiagnostics.CreateScope($"{nameof(ChatThreadClient)}.{nameof(SendMessage)}");
            scope.Start();
            try
            {
                Response<SendChatMessageResult> sendChatMessageResult = _chatThreadRestClient.SendChatMessage(Id, content, priority, senderDisplayName, type, cancellationToken);
                return Response.FromValue(sendChatMessageResult.Value.Id, sendChatMessageResult.GetRawResponse());
            }
            catch (Exception ex)
            {
        ...
        public virtual async Task<Response> UpdateMessageAsync(string messageId, string content, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                return await _chatThreadRestClient.UpdateChatMessageAsync(Id, messageId, content, priority : null, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                scope.Failed(ex);
        ...
        public virtual Response UpdateMessage(string messageId, string content, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                return _chatThreadRestClient.UpdateChatMessage(Id, messageId, content, priority: null, cancellationToken);
            }
            catch (Exception ex)
            {
                scope.Failed(ex);
